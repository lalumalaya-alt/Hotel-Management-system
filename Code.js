...
function addFoodOrder(orderData) {
  try {
    ...
    if (!orderData.items || !Array.isArray(orderData.items)) {
       return { success: false, message: "Invalid payload: Items missing." };
    }
    ...
    orderData.items.forEach(item => {
        ...
        // Match 12-column schema: OrderID, CheckInID, RoomNo, Date, MealPeriod, ItemName, Quantity, Rate, TotalAmount, Status, BilledCheckInID, AddedBy
        rowsToAppend.push([
            orderId,
            orderData.checkInId || '',
            orderData.roomNo || '',
